...
for _, v in {'AntiRagdoll', 'TriggerBot', 'SilentAim', 'AutoRejoin', 'Rejoin', 'Disabler', 'Timer', 'ServerHop', 'MouseTP', 'MurderMystery', 'Health', 'PlayerModel', 'SpinBot', 'Swim', 'Phase', 'GamingChair'} do
	vape:Remove(v)
end
run(function()
	local AimAssist
...
run(function()
	...
	BCR = NoClickDelay:CreateToggle({
        Name = "Remove Block Delay",
        Function = function(callback) 
            if callback then
                task.wait(1)
                ...
                if CpsConstants then
                    local newCPS = BlockCPSLimit.Value == 0 and 1000 or BlockCPSLimit.Value
                        ...
                        for _, v in {'2', '5'} do
                            pcall(function()
                                BCR:Clean(lplr.PlayerGui.MobileUI[v].MouseButton1Down:Connect(function()
                                    if CpsConstants then
                                        local currentValue = BlockCPSLimit.Value == 0 and 1000 or BlockCPSLimit.Value
    ...
    BlockCPSLimit = NoClickDelay:CreateSlider({
        Name = "Block Place Limit",
        Tooltip = "The higher the CPS, the more ghost blocks.",
        Default = 30,
        Min = 12,
        Max = 30,
        Function = function()
...
run(function()
	...
	--Start of random custom modules 
run(function()
	local MelodyExploit
	local Heal
	MelodyExploit = vape.Categories.Kit:CreateModule({ -- same thing here
		Name = "MelodyExploit",
...
run(function()
	local RemoveStatus
	local old
	RemoveStatus = vape.Legit:CreateModule({
		Name = "StatusEffects",
		Tooltip = 'Removes annoying effects (Static, Glooped etc.)',
		Function = function(callback)
			if callback then
...
run(function()
    ...
    local Pack = {Value = "HighResolution1"}
                ...
                toolFunction = function(tool)
                    ...
                    for i,v in pairs(index) do  
                        if(v.name == tool.Name) then
                            ...
                            model2.CFrame = tool2:WaitForChild("Handle").CFrame * v.offset
                            model2.CFrame *= CFrame.Angles(math.rad(0),math.rad(-50),math.rad(0))
                            model2.CFrame *= CFrame.new(0.36,0,-.75)
                            model2.Parent = tool2
                            local weld2 = Instance.new("WeldConstraint",model)
                            weld2.Part0 = model2
                            weld2.Part1 = tool2:WaitForChild("Handle")
    ...
    local function refresh()
        ...
        connection = game:GetService("Workspace").Camera.Viewmodel.ChildAdded:Connect(toolFunction)
        for _, v in pairs(game:GetService("Workspace").Camera.Viewmodel:GetChildren()) do 
            toolFunction(v) 
        end
    end
    ...
    TexturePacks = vape.Categories.Render:CreateModule({
        ...
        Tooltip = 'Library of custom item texturepacks.'
    })
...
run(function()
    ...
    VanessaCharger = vape.Categories.Kit:CreateModule({
        Name = 'VanessaCharger',
        Function = function(callback)
   			if role ~= "owner" and role ~= "coowner" and role ~= "admin" and role ~= "friend" and role ~= "premium" and role ~= "user" then
				vape:CreateNotification("Onyx", "You don’t have access to this.", 10, "alert")
...
run(function() 
	...
	AutoBan = vape.Categories.Kit:CreateModule({
		Name = "AutoKitBan",
...
run(function()
	...
	local Blocks
	local old
	MiloDisguse = vape.Categories.Kit:CreateModule({
		Name = "MiloDisguise",
...
run(function()
    ...
    Miner = vape.Categories.Utility:CreateModule({
        Name = 'AutoMiner',
        Function = function(callback)
            if callback then
                task.spawn(function()
...
run(function()
    ...
    Kaliyah = vape.Categories.Kit:CreateModule({
        Name = 'AutoKaliyah',
        Function = function(callback)
            if callback then
...
run(function()
	...
	local Range
	local Animations
	BetterElder = vape.Categories.Kit:CreateModule({
		Name = 'AutoElder',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TextureRemover = vape.Categories.Render:CreateModule({
		Name = 'TextureRemover',
		Function = function(callback)
			if callback then
				processWorkspace()
...
run(function()
    ...
    local antiweb = vape.Categories.Blatant:CreateModule({
        Name = 'AntiCobweb',
...
run(function()
	local GrimReaperFix
	GrimReaperFix = vape.Categories.Kit:CreateModule({
		Name = 'GrimReaperFix',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HotbarVisuals = vape.Legit:CreateModule({
		["Name"] = 'Hotbar Visuals',
...
run(function()
    ...
    local keywords = {"hacker", "hax", "hack", "cheater", "hacking"}

    -- Robust Chat Sender (Tries both systems)
    local function sendChatMessage(msg)
        ...
        -- Try Legacy System if New System fails/isn't used
    ...
    local function onChatReceived(msg, authorName)
        msg = msg:lower()
        for _, word in ipairs(keywords) do
            if msg:find(word) then
                sendChatMessage("actually " .. authorName .. " its called Exploits")
                break
            end
        end
    end
            ...
            if callback then
                ...
                if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
                    table.insert(chatConnections, TextChatService.MessageReceived:Connect(function(textChatMessage)
                        ...
                        if source and source.UserId ~= LocalPlayer.UserId then
                            local plr = Players:GetPlayerByUserId(source.UserId)
                            if plr then onChatReceived(textChatMessage.Text, plr.Name) end
                        end
                    end))
                else
                    local function connectPlayer(plr)
                        if plr == LocalPlayer then return end
                        table.insert(chatConnections, plr.Chatted:Connect(function(msg)
                            onChatReceived(msg, plr.Name)
                        end))
                    end
                    for _, v in pairs(Players:GetPlayers()) do connectPlayer(v) end
                    table.insert(chatConnections, Players.PlayerAdded:Connect(connectPlayer))
                end
            else
                -- Disconnect
                for _, conn in pairs(chatConnections) do conn:Disconnect() end
...
run(function()
	...
	AutoBerserker = vape.Categories.Blatent:CreateModule({
		Name = 'AutoBerserker',
		Function = function(callback)
			if callback then
